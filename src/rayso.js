...
module.exports = class RaySo {
     ...
     * @param {Object} [options]
          ...
          * @param {(true|false)} [options.debug]
            Show information in the terminal about the iamge generation process. 
            Default is false
    */
		...
		try {
			if (this.localPreview) {
				...
				if (this.debug) {
					...
					console.info('[==========] Successfully generated an image... 👍');
				}
	...
	buildPageUrl(code) {
		try {
			return `https://ray.so/?title=${encodeURIComponent(this.title)}&theme=${
				this.theme
			}&spacing=${this.padding}&background=${this.background}&darkMode=${
				this.darkMode
	...
	validateParameters(params) {
		try {
			...
			if (
				...
				errors.push('Padding parameter must be type of string or number.');
